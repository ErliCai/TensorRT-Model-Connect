...
CHECKPOINTS = {
    "campplus": ("campplus.onnx", "a6ac6a63997761ae2997373e2ee1c47040854b4b759ea41ec48e4e42df0f4d73"),
    "speech_tokenizer": ("speech_tokenizer_v3.onnx", "23236a74175dbdda47afc66dbadd5bcb41303c467a57c261cb8539ad9db9208d"),
}
...
class FrontendWeights:
    ...
    def __init__(self, model_dir, component):
        ...
        if sha256_file(path) != digest:
            raise ValueError(f"Unsupported {filename} checksum; review topology before accepting new weights")
...
class FrontendEngine(ComponentEngine):
    def __init__(self, directory, component, *, device=0):
        ...
        if (self.manifest.get("batch_size") != 1 or self.manifest.get("padded_input") is not False
                ...
                or self.manifest.get("source_sha256", {}).get(filename) != digest):
            raise ValueError("Unsupported frontend semantics or checkpoint")
