...
from tensorrt_model_connect.families.cosyvoice3.checkpoint_mapper import expected_shapes, validate_weights
from tensorrt_model_connect.families.cosyvoice3.config import FlowConfig, ShapeProfile, read_config
from tensorrt_model_connect.families.cosyvoice3.flow_runtime import validate_input_shapes
from tensorrt_model_connect.families.cosyvoice3.constants import time_frequencies
...
def test_package_import_and_help_are_dependency_light():
    code = ("import sys; import tensorrt_model_connect.families.cosyvoice3; "
            "assert 'torch' not in sys.modules; assert 'tensorrt' not in sys.modules")
    subprocess.run([sys.executable, "-c", code], check=True)
    result = subprocess.run([sys.executable, "-m", "tensorrt_model_connect.families.cosyvoice3", "--help"],
...
def test_inspect_cli(tmp_path):
    ...
    result = subprocess.run([sys.executable, "-m", "tensorrt_model_connect.families.cosyvoice3",
...
def test_build_records_implementation_and_workspace(tmp_path, monkeypatch):
    from types import SimpleNamespace
    from tensorrt_model_connect import trt_compat
    from tensorrt_model_connect.families.cosyvoice3 import __main__ as cli
    from tensorrt_model_connect.families.cosyvoice3 import checkpoint_mapper, flow_builder
    ...
    monkeypatch.setattr(flow_builder, "build_flow_engine", fake_build)
    monkeypatch.setattr(cli, "sha256_file", lambda path: "changed" if changed else original_hash(path))
