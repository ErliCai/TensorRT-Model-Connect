...
def component(tmp_path_factory):
    import torch
    from tensorrt_model_connect.families.cosyvoice3.checkpoint_mapper import expected_shapes
    from tensorrt_model_connect.families.cosyvoice3.config import FlowConfig, ShapeProfile
    from tensorrt_model_connect.families.cosyvoice3.flow_builder import build_flow_engine
    from tensorrt_model_connect.families.cosyvoice3.flow_runtime import FlowEngine
    ...
    (path / "manifest.json").write_text(json.dumps({
        ...
        "architecture": asdict(cfg), "profile": asdict(profile), "plan_sha256": hashlib.sha256(plan).hexdigest(),
    }), encoding="utf-8")
