...
from tensorrt_model_connect.families.cosyvoice3.frontend import CHECKPOINTS, FrontendProfile
...
def test_reference_audio_rejects_unsupported_input(tmp_path, seconds, sample_rate):
    ...
    from tensorrt_model_connect.families.cosyvoice3.tts import reference_features
...
def test_prepare_voice_refuses_existing_output(tmp_path):
    ...
    from tensorrt_model_connect.families.cosyvoice3.tts import prepare_voice
...
def test_reference_mel_matches_pinned_matcha(tmp_path, sample_rate):
    ...
    assert sha256_file(path) == "2f741064bfcc9485d19a42356496d8ae24dec80c3d6ef5ca7f4e8abfbf9c61ca"
...
def frontend_pair(request):
    ...
    import torch
    from tensorrt_model_connect.families.cosyvoice3.frontend import FrontendEngine
    from tensorrt_model_connect.families.cosyvoice3.artifacts import sha256_file

    filename, digest = CHECKPOINTS[component]
    assert sha256_file(Path(model) / filename) == digest
...
def compare_frontend(pair, features, case):
    ...
    row = {"component": component, "case": case, "input_shape": list(features.shape),
           "plan_sha256": engine.manifest["plan_sha256"], "shape_equal": actual.shape == expected.shape,
           "checkpoint_sha256": CHECKPOINTS[component][1], "atol": atol, "rtol": rtol,
...
def test_native_voice_preparation(tmp_path, monkeypatch):
    ...
    import soundfile
    from tensorrt_model_connect.families.cosyvoice3.artifacts import sha256_file
    from tensorrt_model_connect.families.cosyvoice3.tts import prepare_voice, read_voice, reference_features
    ...
    for component, key in (("campplus", "speaker_features"), ("speech_tokenizer", "token_features")):
        options = ort.SessionOptions()
        options.intra_op_num_threads = 1
        oracle = ort.InferenceSession(str(model / CHECKPOINTS[component][0]), options, providers=["CPUExecutionProvider"])
    ...
    assert report["voice_sha256"] == sha256_file(output / "voice.npz")
    assert report["audio_sha256"] == sha256_file(audio)
