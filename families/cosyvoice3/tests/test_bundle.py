...
from families.cosyvoice3 import bundle
...
def test_package_includes_current_plan_contents(package_args):
    (package_args.llm / "llm.plan").write_bytes(b"changed")
    bundle.package(package_args)
    assert read_bundle_section(package_args.output, "llm.plan") == b"changed"
