...
def capture(args):
    ...
    manifest = {"scope": "exact_input_native_capture_not_acceptance", "cases": metadata,
                "component_path": str(args.component), "torch_version": torch.__version__,
                "acoustic_evidence_path": str(args.acoustic_evidence) if args.acoustic_evidence else None}
    validate_case_manifest(metadata, has_acoustic=has_acoustic)
    with (args.output / "manifest.json").open("x", encoding="utf-8") as handle:
        json.dump(manifest, handle, indent=2)
    print(json.dumps({"captured": len(cases), "component_path": manifest["component_path"]}))
...
def compare(args):
    ...
    validate_case_manifest(manifest["cases"], has_acoustic=manifest["acoustic_evidence_path"] is not None)
    ...
    report = {"scope": "native_vs_official_in_separate_environment_not_full_tts", "passed": all(row["passed"] for row in rows),
              "source_revision": revision, "native_capture": manifest,
