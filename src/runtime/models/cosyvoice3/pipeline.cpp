...
void require(bool condition, const std::string& message) {
    if (!condition) throw std::runtime_error("CosyVoice3: " + message);
}
template<class T> Tensor tensor(std::vector<T>& values, std::vector<int64_t> shape,
...
std::vector<float> output(const TensorMap& result, const std::string& name,
    ...
    require(t.dtype == DType::kFloat32 && t.shape == shape && t.data,
            "invalid output " + name);
...
TensorMap run(ITrtModule& module, const TensorMap& inputs) {
    ...
    for (const auto& [name, t] : inputs) {
        ...
        for (size_t i = 0; i < lo.size(); ++i)
            require(t.shape[i] >= lo[i] && t.shape[i] <= hi[i], "input outside engine profile: " + name);
        if (t.dtype == DType::kFloat32 && t.numel()) {
            auto values = static_cast<const float*>(t.data);
            require(values && std::all_of(values, values + t.numel(), [](float x) { return std::isfinite(x); }),
...
int sample(const std::vector<float>& logits, const std::vector<int32_t>& history,
    ...
    for (double x : p) require(std::isfinite(x), "nonfinite speech logits");
    if (static_cast<int>(history.size()) < minimum) p[6561] = -std::numeric_limits<double>::infinity();
    auto maximum = std::max_element(p.begin(), p.end());
    if (greedy) return static_cast<int>(maximum - p.begin());
    double top = *maximum, total = 0;
    for (double& x : p) { x = std::exp(x - top); total += x; }
    ...
    double mass = 0;
    size_t count = 0;
    do { mass += p[order[count++]]; } while (mass < .8 && count < 25);
    auto choose = [&](const std::vector<int>& ids, size_t n, double sum) {
        ...
        for (size_t i = 0; i < n; ++i) {
            cumulative += p[ids[i]];
            if (u * sum < cumulative) return ids[i];
        }
        return ids[n - 1];
    };
...
std::vector<int32_t> pack(const ITokenizer& tokenizer, const Settings& settings,
                           const Voice& voice, const std::string& text) {
    plain(text); plain(settings.instruction); plain(settings.transcript, true);
    require(settings.transcript.empty() || !voice.tokens.empty(), "transcript needs reference tokens");
    ...
    for (int id : prompt) { require(id >= 0 && id < 151936, "text token out of range"); ids.push_back(id); }
    ids.push_back(151936 + 6563);
    if (!settings.transcript.empty())
        for (int id : voice.tokens) ids.push_back(151936 + id);
    return ids;
}
...
Pipeline::Pipeline(Settings settings, Voice voice, std::unique_ptr<ITokenizer> tokenizer,
    ...
    for (int id : voice_.tokens) require(id >= 0 && id < 6561, "invalid reference speech token");
    for (float x : voice_.features) require(std::isfinite(x), "nonfinite reference mel");
    double norm = 0;
    for (float x : voice_.speaker) { require(std::isfinite(x), "nonfinite speaker"); norm += double(x) * x; }
    require(norm > 0, "zero speaker embedding");
}

AudioResult Pipeline::generate_audio(const std::string& text, const GenerateConfig& cfg) {
    auto ids = pack(*tokenizer_, settings_, voice_, text);
    ...
    require(limit <= settings_.max_tokens && ids.size() + limit - 1 <= size_t(settings_.max_context),
    ...
    std::vector<int32_t> speech;
    {
        ...
        for (int step = 0; step < limit; ++step) {
            ...
            for (int q = 0; q < n; ++q)
                for (int k = 0; k < total; ++k) mask[size_t(q) * total + k] = k <= past + q;
            auto result = run(*llm, {{"ids", tensor(ids, {1,n}, DType::kInt32)},
                ...
                {"keys", tensor(keys, {24,2,past,64})}, {"values", tensor(values, {24,2,past,64})}});
            ...
            past = total;
            int token = sample(logits, speech, std::min(text_length * 2, limit), settings_.greedy, draw);
            if (token >= 6561) { stopped = true; break; }
            speech.push_back(token);
            ids = {151936 + token};
        }
        require(stopped && !speech.empty(), "LLM stopped empty or reached length limit without EOS");
    }
    const int prompt_frames = static_cast<int>(voice_.tokens.size()) * 2;
    ...
    std::vector<float> mu, spks;
    {
        ...
        auto tokens = voice_.tokens;
        tokens.insert(tokens.end(), speech.begin(), speech.end());
        auto result = run(*conditioner, {{"tokens", tensor(tokens, {1,int64_t(tokens.size())}, DType::kInt32)},
                                        {"speaker", tensor(voice_.speaker, {1,192})}});
    ...
    std::vector<float> mel(size_t(80) * (frames - prompt_frames));
    {
        ...
        mu.resize(2 * plane, 0); spks.resize(160, 0);
        ...
        for (size_t i = 0; i < plane; ++i) x[i] = gaussian(rng);
        for (int c = 0; c < 80; ++c)
            for (int f = 0; f < prompt_frames; ++f) cond[size_t(c) * frames + f] = voice_.features[size_t(f) * 80 + c];
        for (int step = 0; step < 10; ++step) {
            ...
            auto result = run(*flow, {{"x", tensor(x, {2,80,frames})}, {"mask", tensor(mask, {2,1,frames})},
                {"mu", tensor(mu, {2,80,frames})}, {"t", tensor(t, {2})}, {"spks", tensor(spks, {2,80})},
                {"cond", tensor(cond, {2,80,frames})}, {"positions", tensor(positions, {frames}, DType::kInt32)}});
            auto velocity = output(result, "velocity", {2,80,frames});
            for (size_t i = 0; i < plane; ++i) x[i] += dt * (1.7f * velocity[i] - .7f * velocity[plane + i]);
        }
        for (int c = 0; c < 80; ++c)
    ...
    for (float& x : noise) x = std::generate_canonical<float, 24>(rng);
    ...
    audio.num_samples = samples; audio.sample_rate = 24000;
    return audio;
}
} // namespace trtmc::cosyvoice3
